...
void get_orthographic(float left, float right,
...
void Win32GetOpenGlContext(HDC windowDC)
{
...
void ogl_init_shaders(Win32_State *win32State)
{
    ...
    AppendCString(Path + PathLength, "\\..\\..\\tankbattle\\shaders\\vertexshader.vertexshader");
	read_file_result vsFile = win32_read_file_to_ntchar(Path);        
    
    Copy(PathLength, win32State->ExeFilePath, Path);
    AppendCString(Path + PathLength, "\\..\\..\\tankbattle\\shaders\\fragmentshader.fragmentshader");
...
GLuint ogl_init_texture(uint32 textureWidth, uint32 textureHeight, 
...
void open_gl_init(Win32_State *win32State, HWND window)
{
...
void draw_quads(QuadBuffer *buffer)
{	
...
void win32_ogl_render(HDC device_context, RenderBuffer *buffer)
{
	...
	MatrixTranslate44(-24.0f, -14.5f, 5.0f, camera_matrix);
...
void add_quad_to_render_buffer(Quad quad, uint32 texture_handle, RenderBuffer &render_buffer)
{
	int index = -1;
	for (int i = 0; i < render_buffer.buffer_count; i++)
	{
		if (render_buffer.quadBuffers[i]->texture_handle == texture_handle)
		{
			index = i;
			break;
		}
	...
	if (index == -1)
	{
		Assert(render_buffer.buffer_count < QUAD_BUFFER_MAX);
		render_buffer.quadBuffers[render_buffer.buffer_count] 
			= (QuadBuffer *)VirtualAlloc(0, sizeof(QuadBuffer), MEM_COMMIT, PAGE_READWRITE);
		render_buffer.quadBuffers[render_buffer.buffer_count]->texture_handle = texture_handle;
		index = render_buffer.buffer_count;
		render_buffer.buffer_count++;
	}
    
	QuadBuffer * quad_buffer = render_buffer.quadBuffers[index];
	Assert(quad_buffer->quad_count < QUAD_BUFFER_SIZE);
	quad_buffer->quads[quad_buffer->quad_count] = quad;
	quad_buffer->quad_count += 1;
}

void reset_quad_buffers(RenderBuffer * buffer)
{
	for (int i = 0; i < buffer->buffer_count; i++)
	{
