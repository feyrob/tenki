...
{    
    ...
    {
		data->tank_texture = Platform.LoadTexture("..\\..\\tankbattle\\assets\\tank_base.png");
		data->turret_texture = Platform.LoadTexture("../../tankbattle/assets/tank_turret.png");
		data->block_texture = Platform.LoadTexture("../../tankbattle/assets/block.png");
		data->map_tex = Platform.LoadTextureData("../../tankbattle/assets/map.bmp");
		data->bullet_texture = Platform.LoadTexture("../../tankbattle/assets/bullet.png");
...
void RenderGameplay(platform_api *PlatformAPI, Game_Memory *memory, RenderBuffer &render_buffer)
{
	...
	{
		Platform.AddQuadToRenderBuffer(make_quad_from_entity(data->player1.ent), data->tank_texture.handle, render_buffer);
		Platform.AddQuadToRenderBuffer(make_quad(data->player1.ent.pos.x, data->player1.ent.pos.y, 1.0f, 1.0f,
		                                         data->player1.ent.t_rot + data->player1.ent.rotation), data->turret_texture.handle, render_buffer);
	}
	if (data->player2.ent.is_active)
	{
		Platform.AddQuadToRenderBuffer(make_quad_from_entity(data->player2.ent), data->tank_texture.handle, render_buffer);
		Platform.AddQuadToRenderBuffer(make_quad(data->player2.ent.pos.x, data->player2.ent.pos.y, 1.0f, 1.0f,
		                                         data->player2.ent.t_rot + data->player2.ent.rotation), data->turret_texture.handle, render_buffer);
	}
	for (int i = 0; i < data->block_count; i++)
	{
		if (!data->blocks[i].is_active) continue;
		Platform.AddQuadToRenderBuffer(make_quad_from_entity(data->blocks[i]), data->block_texture.handle, render_buffer);
	}

	for (int i = 0; i < NUM_BULLETS; i++)
	{
		if (!data->bullets[i].is_active) continue;
		Platform.AddQuadToRenderBuffer(make_quad_from_entity(data->bullets[i]), data->bullet_texture.handle, render_buffer);
	}
}